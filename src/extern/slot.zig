...
pub inline fn slot(buf_out: []u8, slot_no: u32) i64 {
    return c_slot(@intFromPtr(buf_out.ptr), buf_out.len, slot_no);
}

pub inline fn slot_set(keylet: []const u8, slot_no: u32) i64 {
    return c_slot_set(@intFromPtr(keylet.ptr), keylet.len, slot_no);
}

pub inline fn slot_clear(slot_no: u32) i64 {
    return c_slot_clear(slot_no);
}

pub inline fn slot_count(slot_no: u32) i64 {
    return c_slot_count(slot_no);
}

pub inline fn slot_float(slot_no: u32) i64 {
    return c_slot_float(slot_no);
}

pub inline fn slot_subarray(parent_slot: u32, array_id: u32, new_slot: u32) i64 {
    return c_slot_subarray(parent_slot, array_id, new_slot);
}

const FieldCode = @import("../sfcode.zig").FieldCode;
pub inline fn slot_subfield(parent_slot: u32, field_id: FieldCode, new_slot: u32) i64 {
    return c_slot_subfield(parent_slot, @intFromEnum(field_id), new_slot);
}

pub inline fn slot_type(slot_no: u32) i64 {
    return c_slot_type(slot_no);
}

pub inline fn slot_size(slot_no: u32) i64 {
