...
pub inline fn otxn_id(buf_out: *[32]u8, flags: enum(u32) { originatingTxn = 0, emittedTxn = 1 }) i64 {
    return c_otxn_id(@intFromPtr(buf_out.ptr), buf_out.len, @intFromEnum(flags));
}

pub inline fn otxn_slot(slot_no: u32) i64 {
    return c_otxn_slot(slot_no);
}

pub inline fn meta_slot(slot_no: u32) i64 {
